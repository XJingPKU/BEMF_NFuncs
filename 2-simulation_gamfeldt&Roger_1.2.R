...
for (l in 0:funcnum) {
  set.seed(999)
  # choose method = "average" if no functions with complementarity
  #        method = "comp", otherwise
  if(l == 0) {
    method = "av"
  }  else {
    method = "comp"
  ...
  AvFunc <- AverageFunction(SpecMat, FuncMat,
                            ...
                            compfunc = compfunc,
                            r = r)
  # standardize ecosystem functions using the scale function
  AvFunc <- AvFunc %>% 
    select(Richness, one_of(func.names)) %>% 
    mutate_at(vars(one_of(func.names)), function(x) {
  ...
  for (i in seq_len(funcnum)) { 
    ...
    for (k in seq_len(ncol(func_comb))) { 
      ...
      # calculate standardize function using the scale function
      AvFunc_unit <- AvFunc %>%
        select(Richness, one_of(func_comb[, k])) %>% 
        mutate(meanFunction = rowMeans(.[func_comb[, k]])) %>% 
        mutate(meanFunction = scale(meanFunction, center = TRUE, scale = TRUE))
...
# ggsave("./outputs/simulations_gamfeldt-Roger_stdBYmax.pdf", height = 4.5, width = 8.0)
ggsave("./outputs/simulations_gamfeldt-Roger_stdBYscale.pdf", height = 4.5, width = 8.0)

# Classify the biodiversity effect on EMF into three groups
# significantly positive, neutral and significantly negative
# using 95% confidence interval of the significance tests for linear model
Slope_res <- df
...
ggsave("./outputs/simulation_Significance_tests.pdf", height = 5.0, width = 9.0)

# the fraction of functions with a significantly positive/neutral relationship with diversity
...
df.nfuncs.sig %>%
  ...
  labs(x = "Number of functions", 
       y = "Fraction of positive or neutral \nbiodiversity effect on EMF") +
  theme_bw(base_size = 16.5) +
  theme(panel.background = element_blank(),
        panel.grid = element_blank(),
        legend.title = element_blank(),
        strip.background = element_blank())
ggsave("./outputs/simulation_fraction_significance_tests.pdf", height = 5.0, width = 9.0)
