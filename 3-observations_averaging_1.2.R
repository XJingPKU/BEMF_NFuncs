...
cleanSlopeDF <- function(x, ecosystem.types) {
  ...
  x$std <- rep(c("Averaging approach", "Summing approach", "Scaling approach"),
               dim(x)[1] / 3)
  x$std <- factor(x$std,
                  levels = c("Averaging approach", "Summing approach", "Scaling approach"),
...
# using averaging, summing and scaling approaches

res.df.drylands <- NULL
for (i in 1:length(vars4func.maestre)) {
  N <- dim(combn(vars4func.maestre, i))[2]
  ...
  for (j in J) {
    ...
    for (l in c("av", "sm", "sc")) {
      if (l == "av") {
        ...
        emf <- (emf - mean(emf, na.rm = TRUE)) / sd(emf, na.rm = TRUE)
      }
      df <- cbind(maestre.dat, emf)
      res <- matrix(NA, nrow = 1, ncol = 5)
      mod <- lm(emf ~ SR, data = df)
...
biodepth.dat <- biodepth.dat %>% 
  ...
  dplyr::select(location, fb, latitude, longitude, altitude, 
...
for (i in 1:26) {
  ...
  for (j in J) {
    ...
    for (l in c("av", "sm", "sc")) {
      ...
      # fit the model
      mod <- lm(emf ~ target_species_richness,
                data = df2)
      k <- k + 1
...
# sensitive analysis using Jena data
# using subset data
res.df.jena <- NULL
for (i in 1:100) {
  res1 <- res2 <- res3 <- res4 <- res5 <- res6 <- numeric(length = 9*3)
  k <- 0
  for (j in c(75, 70, 65, 60, 55, 50, 45, 40, 35, 30, 25, 20, 15, 10, 5)) {
    # make an empty data for each loop
    dat <- NULL
    emf <- NULL
    df2 <- NULL
    mod <- NULL
    if (j == 75) {
      vars4func.jenaTemp <- sample(vars4func.jena, j)
      temp <- jena.dat[vars4func.jenaTemp]
      emf <- rowSums(temp) / j
      emf.sd <- apply(temp, 1, sd)  # scaled approach
    } else {
      vars4func.jenaTemp <- sample(vars4func.jenaTemp, j)
      temp <- jena.dat[vars4func.jenaTemp]
      emf <- rowSums(temp) / j
      emf.sd <- apply(temp, 1, sd)
    } 
    for (l in c("av", "sm", "sc")) {
      if (l == "av") {
        emf <- emf
        ...
        # emf <- (emf - mean(emf)) / sd(emf)
        emf <- emf / emf.sd  # scaled approach
...
JenaSensAna <- cleanSlopeDF(res.df.jena, ecosystem.types = "Jena grassland")

ggplot(JenaSensAna, aes(ncombns, coef)) +
  geom_point() +
  geom_smooth(method = "lm") +
  facet_wrap(~ std, ncol = 1, scales = "free") +
  theme_bw()
...
ggplot(res.df.all.prob, aes(x = nfuncs, y = prop, color = sig)) +
  ...
  scale_color_manual(values = c("#fb6a4a", "gray", "#4393c3")) +
  labs(x = "Number of functions considered", 
       y = "Fraction of positive, neutral or negative\nbiodiversity effects on EMF") +
  facet_grid(std ~ ecotypes, scales = "free") +
  theme_bw(base_size = 16.5) +
  theme(strip.background = element_blank(),
        panel.grid = element_blank(),
        legend.position = 'none')
ggsave("./outputs/observations_fraction.pdf", width = 11.65, height = 6.29)
